...
func getHydrateDrinkReminderRequestIndentifiers() -> [String] {
...
func getHydrateDrinkRemindersSubTitles() -> [String] {
...
func setHydrateDrinkReminders(dayStartTime: Date) {
    ...
    let allNotifications = [firstInterval1HourElapsed, firstInterval2HoursElapsed, firstInterval2AndHalfHoursElapsed, firstInterval2AndThreeQuarterHoursElapsed, secondInterval1HourElapsed, secondInterval2HoursElapsed, secondInterval2AndHalfHoursElapsed, secondInterval2AndThreeQuarterHoursElapsed, thirdInterval1HourElapsed, thirdInterval2HoursElapsed, thirdInterval2AndHalfHoursElapsed, thirdInterval2AndThreeQuarterHoursElapsed, fourthInterval1HourElapsed, fourthInterval2HoursElapsed, fourthInterval2AndHalfHoursElapsed, fourthInterval2AndThreeQuaterHoursElapsed, extraTimeStart]
...
//    let todaysDateComponents = dately.dateComponentsFor(date: dayStartTime)
    ...
    for reminder in allNotifications {
        reminder.title = "Time To Hydrate"
...
//        }
        ...
        center.removePendingNotificationRequests(withIdentifiers: seeWhichNotificationsShouldFire(currentDate:  Date(), startOfDay:  hydrateManager.lastDayStarted))
...
func getIDsToCancelForIntervalsWithGoalAchieved(intervalsToCancel: [Int]) -> [String] {
    ...
    if intervalsToCancel.contains(4) {
        tempIDsToCancel.append(contentsOf: ["fourthIntervalStart", "fourthInterval1HourElapsed", "fourthInterval2HoursElapsed", "fourthInterval2AndHalfHoursElapsed", "fourthInterval2AndThreeQuaterHoursElapsed"])
    }
...
func compareTotalToIntervals(manager: HydrateManager) -> [Int] {
    ...
    if total >= manager.interval2Goal {
        intervalsToCancel.append(2)
    }
    if total >= manager.interval3Goal {
        intervalsToCancel.append(3)
    }
    if total >= manager.interval4Goal {
